...
describe('settings: health banner', () => {
  ...
  describe('Outlook connection state', () => {
    it('reports the Microsoft connection as not set up on a fresh install', async () => {
      const res = await request(ctx.app).get('/api/settings');
      expect(res.body.microsoftConnected).toBe(false);
      expect(res.body).not.toHaveProperty('emailProvider');
      expect(res.body).not.toHaveProperty('googleConnected');
    });
  });
});
