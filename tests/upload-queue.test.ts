...
import { WaveAPIError } from '../server/services/wave.js';
 ...
 */
vi.mock('../server/services/wave.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../server/services/wave.js')>();
...
async function setupQueue(envOverrides: Record<string, string | undefined> = {}) {
  ...
  const waveModule = await import('../server/services/wave.js');
  const queueModule = await import('../server/services/upload-queue.js');
