...
describe('Gmail message parsing', () => {
  ...
  beforeEach(async () => {
    ctx = await setupTestApp();
    gmail = await import('../../server/integrations/google/gmail.js');
  });
  afterEach(() => {
    vi.unstubAllGlobals();
    ctx.teardown();
...
describe('matching a request to a calendar event', () => {
  let ctx: TestContext;
  let calendar: typeof import('../../server/integrations/google/calendar.js');

  beforeEach(async () => {
    ctx = await setupTestApp();
    calendar = await import('../../server/integrations/google/calendar.js');
  });
  afterEach(() => ctx.teardown());
